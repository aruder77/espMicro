...
class OTAUpdater:
    ...
    def check_for_update_to_install_during_next_reboot(self) -> bool:
        ...
        if latest_version[1:] > current_version[1:]:
            self.logger.info('New version available, will download and install on next reboot')
    ...
    def install_update_if_available(self) -> bool:
        ...
        (current_version, latest_version) = self._check_for_new_version()
        if latest_version[1:] > current_version[1:]:
            self.logger.info('Updating to version {}...'.format(latest_version))
            self._create_new_version_file(latest_version)
    ...
    def _using_network(ssid, password):
        ...
        if not sta_if.isconnected():
            _logger.info('connecting to network...')
            sta_if.active(True)
    ...
    def _download_new_version(self, version):
        _logger.info('Downloading version {}'.format(version))
        self._download_all_files(version)
        _logger.info('Version {} downloaded to {}'.format(
    ...
    def _download_all_files(self, version, sub_dir=''):
        ...
        for file in file_list.json():
            ...
            if file['type'] == 'file':
                gitPath = file['path']
                _logger.info('\tDownloading: %s to %s' % (gitPath, path))
                self._download_file(version, gitPath, path)
            elif file['type'] == 'dir':
                _logger.info('Creating dir', path)
                self.mkdir(path)
    ...
    def _copy_secrets_file(self):
        if self.secrets_file:
            ...
            _logger.info('Copying secrets file from {} to {}'.format(fromPath, toPath))
            self._copy_file(fromPath, toPath)
            _logger.info('Copied secrets file from {} to {}'.format(fromPath, toPath))

    def _delete_old_version(self):
        _logger.info('Deleting old version at {} ...'.format(
            self.modulepath(self.main_dir)))
        self._rmtree(self.modulepath(self.main_dir))
        _logger.info('Deleted old version at {} ...'.format(
            self.modulepath(self.main_dir)))

    def _install_new_version(self):
        _logger.info('Installing new version at {} ...'.format(
            self.modulepath(self.main_dir)))
        ...
        _logger.info('Update installed, please reboot now')
